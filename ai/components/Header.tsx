        ...
        {currentUser && (
            <div className="absolute top-1/2 right-0 -translate-y-1/2 flex items-center gap-4">
                <div className="hidden sm:flex items-center gap-4">
                    <div className="bg-gray-900/50 border border-gray-700 rounded-full px-4 py-1.5 text-sm flex items-center gap-4">
                        <div>
                            <span className="text-gray-400">User: </span>
                            <span className="font-medium text-gray-200">{currentUser.email}</span>
                        </div>
                        <div className="w-px h-4 bg-gray-600"></div>
                        <div>
                            <span className={`font-bold ${currentUser.role === 'admin' ? 'text-yellow-400' : 'text-blue-400'}`}>{currentUser.credits}</span>
                            <span className="text-gray-400"> credits</span>
                        </div>
                    </div>

                    {currentUser.role === 'admin' && (
                        <a href="/#admin" className="text-yellow-400 hover:text-yellow-300 transition-colors text-sm font-medium">
                            Admin Panel
                        </a>
                    )}
                </div>
                ...
                <button
                    onClick={onLogout}
                    className="bg-gray-700 hover:bg-gray-600 text-white text-sm font-medium rounded-md p-2 transition-colors"
                >
                    Logout
                </button>
            </div>
